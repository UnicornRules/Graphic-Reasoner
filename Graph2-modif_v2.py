                                            ...
                                            for Y in B2r2Y[A2][t]:
                                                if Original:
                                                    if (Original, Y) in dict_ax.keys():
                                                        dict_ax(Original, Y).add(
                                                            ((Original, r, A1), (A1, s, A2), (t, A2, Y)))
                                                    else:
                                                        dict_ax[(Original, Y)] = {
                                                            ((Original, r, A1), (A1, s, A2), (t, A2, Y))}
                                                else:
        ...
        if '__' in A1:
            ...
            if A in S.processed_pool.K2rH.keys() and r in S.processed_pool.K2rH[A].keys():
                for X in S.processed_pool.K2rH[A][r]:
                    ...
                    list_connection_3 = function_5(X, A, r, s2rt, S.ontology_pool.B2rA,
                                                   S.processed_pool.K2rH)  # H3 with (A1,A2,S) -> (X,r,t,Y)
                    ...
                    list_connection_1 = function_4(r, A, S.processed_pool.H2rK, S.processed_pool.K2rH, r2st,
                                                   S.ontology_pool.B2rA)
            ...
            for elem in list(dic_init[A1]):
                ...
                if '__' in elem:
                    ...
                    A2 = split[0]
                    s = split[1]
                    list_connection_1 = function_4(s, A2, S.processed_pool.H2rK, S.processed_pool.K2rH, s2rt,
                                                   S.ontology_pool.B2rA, A1)
...
def solving_normalization_issues(axiom_dict,property_dict):
    for keys in list(axiom_dict.keys()):
        for elem in list(axiom_dict[keys]):
                if '__' in elem:
                    split = elem.split('__')
                    A=split[0]
                    r=split[1]
                    if r in property_dict.keys():
                        for s in property_dict[r]:
                            new_ax = A+'__'+s
                            axiom_dict[keys].add(new_ax)
        if '__' in keys:
            split = keys.split('__')
            A=split[0]
            r=split[1]
            if r in property_dict.keys():
                for s in property_dict[r]:
                    new_ax=A+'__'+s
                    if new_ax in axiom_dict.keys() and elem not in axiom_dict[new_ax]:
                        for elem in list(axiom_dict[keys]):
                            axiom_dict[new_ax].add(elem)
                    elif new_ax not in axiom_dict.keys():
                        axiom_dict[new_ax] = axiom_dict[keys]
    return(axiom_dict)
...
if __name__ == '__main__':
    ...
    for i, file in enumerate(dirs):
        ...
        #print(S2.processed_pool.H2A['galen7.owl#IntraAbdominalSystemicArtery'])  
        list_dict.append(S2)
        # print('a')
    ...
    print(longueur(S2.ontology_pool.A2B))
